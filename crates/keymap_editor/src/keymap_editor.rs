...
use rdg_actions::{ChangeKeybinding, OpenKeymap};
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("edit-in-json", "Edit in JSON")
                                            .key_binding(
                                                ui::KeyBinding::for_action_in(&rdg_actions::OpenKeymapFile, &focus_handle, cx)
                                                    .map(|kb| kb.size(rems_from_px(10_f32))),
                                            )
                                            .on_click(|_, window, cx| {
                                                window.dispatch_action(
                                                    rdg_actions::OpenKeymapFile.boxed_clone(),
                                                    cx,
                                                );
                                            })
                                    )
...
mod tests {
    ...
    fn binding_is_unbound_by_unbind_respects_precedence() {
        let binding = gpui::KeyBinding::new("tab", rdg_actions::OpenKeymap, None);
        let unbind =
