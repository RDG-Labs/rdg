...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../rdg/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Rdg"),
        "preview" => ("app-icon-preview.ico", "Rdg Preview"),
        "nightly" => ("app-icon-nightly.ico", "Rdg Nightly"),
        _ => ("app-icon-dev.ico", "Rdg Dev"),
    };
