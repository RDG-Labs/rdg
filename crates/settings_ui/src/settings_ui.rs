...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &rdg_actions::assistant::OpenSkillCreator, cx| {
        open_skill_creator(pages::SkillCreatorOpenMode::Form, None, cx);
    });
    cx.on_action(|_: &rdg_actions::assistant::CreateSkillFromUrl, cx| {
    ...
    cx.observe_new(|workspace: &mut workspace::Workspace, _, _| {
        workspace
            ...
            .register_action(
                |_, _: &rdg_actions::assistant::OpenSkillCreator, window, cx| {
                    let window_handle = window.window_handle().downcast::<MultiWorkspace>();
                    open_skill_creator(pages::SkillCreatorOpenMode::Form, window_handle, cx);
                },
            )
            .register_action(
                |_, _: &rdg_actions::assistant::CreateSkillFromUrl, window, cx| {
                    let window_handle = window.window_handle().downcast::<MultiWorkspace>();
                    let initial_url = pages::skill_url_from_clipboard(cx);
                    open_skill_creator(
...
pub mod test {
    ...
    async fn test_open_skill_creator_action_opens_settings_window_at_sub_page(
        ...
        multi_workspace.update_in(cx, |_multi_workspace, window, cx| {
            window.dispatch_action(Box::new(rdg_actions::assistant::OpenSkillCreator), cx);
        });
