...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Rdg failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Rdg failed to open a window: {e:?}. See https://github.com/RDG-Labs/rdg/blob/main/docs/src/linux.md for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Rdg failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://github.com/RDG-Labs/rdg/blob/main/docs/src/linux.md for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("Rdg System Specs (from CLI):\n{}", system_specs);
        return;
    }
